...
def get_custom_strategy():
    ...
    # 스타일 매핑 로드 (임시로 API에서 가져오기)
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 전략 설정용으로는 로컬 파일 사용 (빠른 로딩)
    sp500_df = load_sp500_data_with_fallback(os.path.join(script_dir, 'sp500_data.xlsx'))
    if sp500_df is None:
        return None
...
def run_rotation_strategy(df, strategy_name, strategy_rules, seasons, initial_capital=10000000, special_strategy=None):
    ...
    # 특별 조건이 있으면 Buy & Hold 최적화 비활성화
    unique_styles = set(strategy_rules.values())
    is_buy_and_hold = len(unique_styles) == 1 and special_strategy is None
    
    # 특별 조건을 위한 계절 히스토리 추적
    season_history = []
    in_sideways_mode = False  # 횡보 모드 상태
    ...
    if is_buy_and_hold:
        ...
        for i, date in enumerate(df.index):
            ...
            if pd.isna(season):
                ...
                # 계절 히스토리 업데이트
                season_history.append(season)
                if len(season_history) > 5:  # 최근 5개 계절 추적
                    season_history.pop(0)
                
                # 횡보 모드 진입/종료 확인
                if special_strategy:
                    # 가을 → 봄 → 가을 패턴으로 횡보 모드 진입
                    if not in_sideways_mode and len(season_history) >= 3:
                        if (season_history[-3] == '가을' and 
                            season_history[-2] == '봄' and 
                            season_history[-1] == '가을'):
                            in_sideways_mode = True
                            print(f"[특별조건] {date.strftime('%Y-%m')}: 횡보 모드 진입 (가을→봄→가을)")
                    
                    # 횡보 모드 종료 조건: 여름 또는 겨울 출현 시
                    if in_sideways_mode and season in ['여름', '겨울']:
                        in_sideways_mode = False
                        print(f"[특별조건] {date.strftime('%Y-%m')}: 횡보 모드 종료 ({season} 진입)")
                
                # 스타일 결정
                if in_sideways_mode and special_strategy and season in special_strategy:
                    target_style = special_strategy[season]
                    print(f"[특별조건] 횡보 모드 - {season} → {target_style}")
                else:
...
def main():
    ...
    # 1. 데이터 로딩 (API 우선, 실패 시 로컬 파일)
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    print("=== 데이터 로딩 방식 선택 ===")
    data_source = input("데이터 소스 선택 (1: API 우선, 2: 로컬 파일만, 기본값: 1): ").strip()
    ...
    if data_source == '2':
        ...
        # API 우선, 실패 시 로컬 파일
        sp500_df = load_sp500_data_with_fallback(os.path.join(script_dir, 'sp500_data.xlsx'))
        rsi_series = load_rsi_data_with_fallback(os.path.join(script_dir, 'RSI_DATE.xlsx'))
    
    if sp500_df is None:
        print("S&P 500 데이터를 로딩할 수 없습니다.")
        return
    
    if rsi_series is None:
        print("RSI 데이터를 로딩할 수 없습니다.")
        return
