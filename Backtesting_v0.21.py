        ...
        if use_special == 'y':
            ...
            while True:
                try:
                    ...
                    if choice in style_mapping:
                        special_strategy['봄'] = style_mapping[choice]
                        print(f"  ✓ 횡보 구간 봄: {style_mapping[choice]}")
                        break
                    else:
                        print("잘못된 번호입니다. 다시 선택해주세요.")
                except ValueError:
            ...
            while True:
                try:
                    ...
                    if choice in style_mapping:
                        special_strategy['가을'] = style_mapping[choice]
                        print(f"  ✓ 횡보 구간 가을: {style_mapping[choice]}")
                        break
                    else:
                        print("잘못된 번호입니다. 다시 선택해주세요.")
                except ValueError:
...
def run_rotation_strategy(df, strategy_name, strategy_rules, seasons, initial_capital=10000000, special_strategy=None):
    """로테이션 전략을 실행합니다. (투자 시차 및 계절별 통계 수정 최종 버전)"""
    portfolio_values = []
    transactions = []
    current_style = None
    current_shares = 0
    ...
    for i, date in enumerate(df.index):
        current_season = seasons.loc[date] if date in seasons.index else None
        
        # 포트폴리오 가치 계산
        # 리밸런싱 전에, '이전' 스타일 보유에 대한 현재 가치를 먼저 계산
        if current_style and current_shares > 0:
            portfolio_value = current_shares * df.loc[date, current_style]
        else:
            portfolio_value = cash

        # 이전 달의 수익률을 해당 계절 통계에 추가
        if i > 0 and prev_season:
            prev_value = portfolio_values[-1]
            if prev_value > 0:
                period_return = (portfolio_value / prev_value) - 1
                season_stats[prev_season].append(period_return)

        # 실제 투자에 사용할 스타일은 '이전 달'의 계절을 기준으로 결정
        target_style = None
        if prev_season:
            # 횡보 모드 관련 로직은 이전과 동일
            if special_strategy:
                season_history.append(prev_season)
                if len(season_history) > 5: season_history.pop(0)
                if not in_sideways_mode and len(season_history) >= 3 and (season_history[-3:] == ['가을', '봄', '가을']):
                    in_sideways_mode = True
                if in_sideways_mode and prev_season in ['여름', '겨울']:
                    in_sideways_mode = False
            
            # 스타일 결정
            if in_sideways_mode and special_strategy and prev_season in special_strategy:
                target_style = special_strategy[prev_season]
            else:
                target_style = strategy_rules[prev_season]
        ...
        if target_style and target_style != current_style:
            ...
            if current_style and current_shares > 0:
                ...
                transactions.append({'date': date, 'season': prev_season, 'action': '매도', 'from_style': current_style, 'to_style': None, 'shares': current_shares, 'price': sell_price, 'value': cash})
            
            # 매수
            buy_price = df.loc[date, target_style]
            current_shares = cash / buy_price
            current_style = target_style
            cash = 0
            action_type = '초기투자' if not transactions else '매수'
            transactions.append({'date': date, 'season': prev_season, 'action': action_type, 'from_style': None, 'to_style': current_style, 'shares': current_shares, 'price': buy_price, 'value': current_shares * buy_price})
...
def run_strategy_comparison(sp500_df, seasons, custom_strategy_name, custom_strategy_rules, initial_capital, special_strategy=None):
    ...
    for strategy_name, strategy_rules in all_strategies.items():
        ...
        current_special = special_strategy if strategy_name == custom_strategy_name else None
        
        portfolio_series, transactions, season_stats = run_rotation_strategy(
            sp500_df, strategy_name, strategy_rules, seasons, initial_capital, current_special
        )
...
def main():
    ...
    try:
        ...
        results = run_strategy_comparison(
            ...
            custom_strategy_rules, user_input['initial_capital'], special_strategy
        )
    ...
    # ▼▼▼▼▼ 단일 월 검증 코드 호출 ▼▼▼▼▼
    # 검증하고 싶은 연도와 월을 설정하세요
    debug_single_month_return(1999, 2, sp500_df, rsi_series, custom_strategy_rules)
    debug_single_month_return(1999, 3, sp500_df, rsi_series, custom_strategy_rules)
    # ▲▲▲▲▲ 단일 월 검증 코드 호출 ▲▲▲▲▲
...
def debug_single_month_return(year, month, sp500_df, rsi_series, strategy_rules):
    """특정 월의 수익률 계산 과정을 상세히 출력하여 검증합니다."""
    
    import pandas as pd
    
    # 1. 해당 월과 이전 월의 마지막 날짜 찾기
    current_month_end = pd.Timestamp(year, month, 1) + pd.offsets.MonthEnd(0)
    prev_month_end = current_month_end - pd.offsets.MonthBegin(1) + pd.offsets.MonthEnd(0)

    # 데이터프레임에서 실제 거래일 찾기 (휴일 등을 고려)
    try:
        date_end = sp500_df.index[sp500_df.index.get_loc(current_month_end, method='ffill')]
        date_start = sp500_df.index[sp500_df.index.get_loc(prev_month_end, method='ffill')]
    except KeyError:
        print(f"오류: {year}-{month} 기간의 데이터를 찾을 수 없습니다.")
        return
        
    print(f"\n--- {year}년 {month}월 수익률 계산 검증 ---")
    
    # 2. 이전 월의 RSI와 계절 확인
    try:
        # 날짜에 해당하는 RSI 값을 정확히 찾기
        rsi_date_loc = rsi_series.index.get_loc(date_start, method='ffill')
        prev_rsi = rsi_series.iloc[rsi_date_loc]
        prev_season = classify_market_season(pd.Series([prev_rsi])).iloc[0]
        print(f"1. 신호 확인 ({rsi_series.index[rsi_date_loc].date()} 기준): RSI {prev_rsi:.2f} -> '{prev_season}'")
    except KeyError:
        print(f"오류: {date_start.date()}의 RSI 데이터를 찾을 수 없습니다.")
        return

    # 3. 투자할 스타일 결정
    target_style = strategy_rules[prev_season]
    print(f"2. 투자 스타일 결정: '{target_style}'")
    
    # 4. 시작일과 종료일의 가격 확인
    try:
        price_start = sp500_df.loc[date_start, target_style]
        price_end = sp500_df.loc[date_end, target_style]
        print(f"3. 가격 확인: 시작가({date_start.date()}) {price_start:.3f} | 종가({date_end.date()}) {price_end:.3f}")
    except KeyError:
        print(f"오류: '{target_style}'의 가격 데이터를 찾을 수 없습니다.")
        return
        
    # 5. 수익률 계산 및 비교
    calculated_return = (price_end / price_start) - 1
    print(f"4. 코드 계산 수익률: {calculated_return:.2%}")
    print("=" * 40)
...
def debug_single_month_return(year, month, sp500_df, rsi_series, strategy_rules):
    ...
    # 2. 이전 월의 RSI와 계절 확인
    try:
        prev_rsi = rsi_series.loc[date_start]
        prev_season = classify_market_season(pd.Series([prev_rsi])).iloc[0]
        print(f"1. 신호 확인 ({date_start.date()} 기준): RSI {prev_rsi:.2f} -> '{prev_season}'")
    except KeyError:
        print(f"오류: {date_start.date()}의 RSI 데이터를 찾을 수 없습니다.")
        return
